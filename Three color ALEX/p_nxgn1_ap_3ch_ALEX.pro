...
for i = 0, film_l - 1 do begin
	...
	readu, 1, frame
	if (i mod 2) eq 0 then temp_back=back(j) else temp_back=back2(j) ;frank, odd frames with the green laser on and even frames with the red laser on
	for j = 0, no_good - 1 do begin
		apeak = g_peaks(whc_gpk(j,0),whc_gpk(j,1),*,*) * float(frame(flgd(0,j)-3:flgd(0,j)+3,flgd(1,j)-3:flgd(1,j)+3)-temp_back)
