...
; generate red background using N-19 to N-10 red frames when the red laser is on
openr, 1, run + ".pma"
...
for j = 0, 2*ffilm_l - 1 do begin
   if((j mod 5) eq 0) then print, j, film_l
   readu, 1, frame1
   ave_arr1 = ave_arr1 + frame1
endfor
close, 1
ave_arr1 = ave_arr1/float(2*ffilm_l)
...
; generate green background using N-9 to N green frames when the green laser is on
openr, 1, run + ".pma"
...
close, 1
ave_arr2 = ave_arr2/float(2*ffilm_l)
...
for i = 20, 149 do begin
    for j = 25, 486 do begin
         ...
         if x eq 0 then begin
          if y eq 0 then begin
              ...
              if quality eq 1 then begin
                 ...
                 back(no_good) = aves2(x,y) ;frank for Cy3 background subtraction at green illumination
                 back2(no_good) = aves1(x,y) ;frank for Cy3 background subtraction at red illumination
                 no_good = no_good + 1
                 good(0,no_good) = xf
                 good(1,no_good) = yf
                 back(no_good) = aves2(int_xf,int_yf) ;frank for Cy5 background subtraction at green illumination
                 back2(no_good) = aves1(int_xf,int_yf) ;frank for Cy5 background subtraction at red illumination
                 no_good = no_good + 1
                 good(0,no_good) = xf2
                 good(1,no_good) = yf2
                 back(no_good) = aves2(int_xf2,int_yf2) ;frank for Cy7 background subtraction at green illumination
                 back2(no_good) = aves1(int_xf2,int_yf2) ;frank for Cy7 background subtraction at red illumination
                 no_good = no_good + 1
              endif
          endif
         endif
